...
strategies_all = { 'Strategy A', 'Strategy B', 'Strategy C', 'Strategy D', 'Strategy E', 'Strategy F', 'Strategy G', 'Strategy H', 'Strategy I'};
strategies_subset = {'Strategy A', 'Strategy D', 'Strategy G', 'Strategy H', 'Strategy I'};

% Define strategy sets
strategy_sets_all = {'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I'};
strategy_sets_subset = {'A', 'D', 'G', 'H', 'I'};

colors_all = { ...
    'k', [0.5, 0.5, 0.5], [0.8, 0.8, 0.8], ...  % BHO colors
    [0.5, 0.25, 0], [0.6, 0.3, 0], [0.8, 0.4, 0], ...  % CHO colors
    'b', 'r', 'g'  % DCHO and Proposed DCHO colors
};
colors_subset = {'k', [0.5, 0.25, 0], 'b', 'r', 'g'};
lineStyles_all = { ...
    ':', ':', ':', ...  % BHO line styles
    '-.', '-.', '-.', ...  % CHO line styles
...
for i = 1:length(strategies_all)
    data_path = fullfile('MasterResults', ['case 1','_MASTER_RESULTS_', strategies_all{i}, '_DenseUrban', '.mat']);
...
% Subplot 8: Average HO per UE (Subset Strategies)
subplot(4, 2, 8);
hold on;
for i = 1:length(strategies_subset)
    x_values = 0:100:25000;
    plot(x_values, ho_data_subset{i}, 'Color', colors_subset{i}, 'LineStyle', lineStyles_subset{i}, 'LineWidth', 0.7, 'DisplayName', strategies_subset{i});
end
xlabel('Distance [m]');
ylabel('Average HO');
legend('Location', 'northwest');
