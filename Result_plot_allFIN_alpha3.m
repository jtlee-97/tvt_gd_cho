...
% Subplot 3: UHO 횟수 (전체 UHO 발생 횟수 플롯)
subplot(4, 2, 3);
hold on;
for i = 1:length(strategies_subset)
    % UHO 데이터만 사용하여 CDF 플롯
    [cdf_uho, x_uho] = ecdf(uho_data_subset{i});
    plot(x_uho, cdf_uho, 'Color', colors_subset{i}, 'LineStyle', lineStyles_subset{i}, 'LineWidth', 1.5, 'DisplayName', strategies_subset{i});
end
hold off;
...
% Subplot 7: Bar plot for Average UHO (Subset Strategies)
subplot(4, 2, 7);
...
bar(x, average_uho_all, bar_width, 'FaceColor', [0.8500, 0.3250, 0.0980]); % Orange bars
ylabel('Average UHO');  % y축 레이블을 UHO로 설정

% Set x-axis labels and title
set(gca, 'XTick', x, 'XTickLabel', strategy_sets_subset);  % 전략 레이블 설정
xlabel('Strategy');
grid on;
subplot(4, 2, 8);
...
% Create positions for grouped bars
bar_width = 0.4;
x = 1:length(strategies_all);  % 모든 전략에 대해 x 값 설정

% Create a bar plot for Average SINR
bar(x, average_sinr_all, bar_width, 'FaceColor', [0.8500, 0.3250, 0.0980]); % Orange bars for Average SINR
ylabel('Average SINR [dB]');  % y축 레이블을 SINR로 설정

% Set x-axis labels and title
set(gca, 'XTick', x, 'XTickLabel', strategy_sets_all);  % 모든 전략에 대한 레이블 설정
xlabel('Strategy');
grid on;
